...
```{r table_1}
teachers[ , .(`Remain in Same School` = sum(!move_school_next)/.N,
              `Change Schools Within District` = sum(move_within_next)/.N,
              `Switch Districts` = sum(move_district_next)/.N,
              `Exit Wisconsin Public Schools` = sum(quit_next)/.N,
              `Number of Teachers` = .N),
          keyby = .(`Teacher Experience` = exp_split)]
#table 1b
teachers[ , .(`Teacher Experience` = 'All',
              `Remain in Same School` = sum(!move_school_next)/.N,
              `Change Schools Within District` = sum(move_within_next)/.N,
              `Switch Districts` = sum(move_district_next)/.N,
              `Exit Wisconsin Public Schools` = sum(quit_next)/.N,
              `Number of Teachers` = .N)]
...
teachers[ , {
  ...
  N_move = sum(idx)
  c(as.list(table2(urbanicity_s_next[idx], prop = TRUE)),
    list(`# Changing Districts` = N_move, 
         `% of origin teachers` = N_move/.N))
}, keyby = urbanicity_s]
