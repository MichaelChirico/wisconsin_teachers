...
teachers[districts, 
         `:=`(pct_prof = (n_prof + n_advn)/n_tested, pct_hisp = pct_hisp, 
              pct_black = pct_black, pct_frl = pct_frl,
              urbanicity_d = urbanicity), 
         on = c('year', district_fill = 'district')]

teachers[districts, 
         `:=`(pct_prof_next = (n_prof + n_advn)/n_tested, 
              pct_hisp_next = pct_hisp, pct_black_next = pct_black, 
              pct_frl_next = pct_frl, urbanicity_d_next = urbanicity), 
...
teachers[move_district_next & total_exp_floor <= 10, 
         .(d_lsalary = mean(schedule_lsalary_next, na.rm = TRUE) - 
             mean(schedule_lsalary_next_cf, na.rm = TRUE), 
           d_lsalary_resid = mean(schedule_lsalary_resid_next, na.rm = TRUE) - 
             mean(schedule_lsalary_resid_next_cf, na.rm = TRUE),
           d_pct_prof = mean(pct_prof_next, na.rm = TRUE) - 
             mean(pct_prof, na.rm = TRUE),
           d_pct_hisp = mean(pct_hisp_next, na.rm = TRUE) - 
             mean(pct_hisp, na.rm = TRUE),
           d_pct_black = mean(pct_black_next, na.rm = TRUE) - 
             mean(pct_black, na.rm = TRUE),
           d_pct_frl = mean(pct_frl_next, na.rm = TRUE) - 
