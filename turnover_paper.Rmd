...
<!-- INCLUDE A PARAGRAPH SUMMARIZING RESULTS OF COBS -->
...
The State of Wisconsin's Department of Public Instruction (DPI) releases annual Salary, Position & Demographic reports through the WISEstaff data collection system. These reports represent "a point-in-time collection of all staff members in public schools as of the 3rd Friday of September..." [@dpi], and will serve as the primary source of data on teachers in this paper. Data are available at the position-teacher level cross-sectionally, with each entry corresponding to one of possibly several positions/assignments held by each school district employee[^multiple_positions]. Identifiers in each file permit unique identification of an employee within a given year, but this identifier does not follow teachers between years[^file_number]. To overcome this substantial hurdle to identifying teacher mobility, data are first fed through the matching algorithm described in further detail in the appendix. Essentially, we are aided by the availability of various imperfect identifiers which are more stable over time, most crucially teachers' first and last names and year of birth. By building on these covariates and incorporating some limited fuzzy matching techniques, we construct a panel of teachers spanning the 1994-95 academic year (AY) through AY2015-16 [^spring] consisting of `r pn(total_observations)` teacher-position-year observations. The matching algorithm necessitates elimination of `r pn(total_observations - total_matchable)` (`r to.pct(1 - total_matchable/total_observations, 1L)`%) observations on account of belonging to teachers who could not be uniquely identified in a given year of data due to exact overlap of their first name, last name, and birth year fields with another teacher in the data.

[^multiple_positions]: Many teachers (and other district employees) serve in multiple roles within a school/district, for example as a coach, part-time program aide, or department head. Each of these is filed as a separate observation in the DPI system.
...
X = mwk[ , .(med_ba = median(salary[highest_degree == 4]),
         ...
         ][dcast(payscales[district_fill == 3619], 
                 year + district_fill + tenure ~ highest_degree,
                 value.var = 'wage'),
             on = c('year', 'tenure')]
...
Perhaps more telling is the goodness of fit in minimally small districts. Four such examples are featured in Figure <!--\ref{fig:small_fit}-->. These four districts just barely satisfy the sample restriction that at least 20 teachers be present in both the BA and MA pay track (Melrose-Mindoro and Shell Lake have exactly 20 in each); for this reason, rather than plot the empirical median, we simply present the full distribution of wage and experience in these districts. Here again the COBS fit captures the essence of the wage-tenure curve even in these sparsely-staffed districts. While one can imagine Shell Lake or Waterford Union's actual contract schedule to feature the sort of late-career leveling that we have seen elsewhere, the power to identify and separate such features distinctly from those captured is strongly limited by the lack of data.

```{r small_dist_fit, fig.cap = '\\label{fig:small_fit}Estimation Results for Sparse Districts', eval = FALSE}
# some districts disappeared
par(mfrow = c(2, 2),
    mar = c(0.1, 0.1, 0.6, 0.1),
    oma = c(5.1, 5.1, 4.1, 2.1))
parm = list(x = list(),
...
ymx = 1.05*max(payscales[district_fill %in% dists & 
                           highest_degree == 5L, wage],
               teachers_ps[district_fill %in% dists, salary])
for (ii in 1:4) {
  dst = dists[ii]
  payscales[district_fill == dst, {
    idx = highest_degree == 4L
    matplot(tenure, cbind(wage[idx], wage[!idx]),
            col = c('blue', 'red'), lty = 1L, type = 'l',
            axes = FALSE, ylim = c(0, ymx))
  }]
  teachers_ps[district_fill == dst,
...
true_sched[ , district_fill := sprintf('%04d', district)]
true_sched[ , highest_degree := 4L + (certification == 'MA')]
dyr_com = unique(true_sched[ , .(district_fill, district_name, year)])

par(mfrow = c(1L, 3L), 
    mar = c(.1, .1, .6, .1), 
    oma = c(5.1, 5.1, 4.1, 2.1))
ymx = max(true_sched[dyr_com, wage, on = c('district_fill', 'year')],
          payscales[dyr_com, wage, on = c('district_fill', 'year')])
parm = list(x = list(),
            y = list(las = 1L))
for(ii in seq_len(nrow(dyr_com))) {
  this_dist = dyr_com[ii]
  is_monona = this_dist[ , unique(district_fill)] == '3675'
  if (is_monona) {
  ...
  if (ii == 1L) mtext('Salary', side = 2, line = 4)
  mtext('Tenure', side = 1, line = 3)
...
err = payscales[true_sched, x.wage - i.wage,
                on = c('district_fill', 'year', 'highest_degree', 'tenure')]
