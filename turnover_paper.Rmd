...
```{r table_1, warning = FALSE, results = 'asis'}
library(funchir)
T1 = rbind(
...
tbl = capture.output(print(xtable(
  ...
  sanitize.text.function = identity, floating.environment = 'longtable'))
idx = grep('^Origin', tbl)
...
tbl = c(tbl[1L:(idx - 1L)],
        paste0(paste('', mcol('Men by Experience Class', 3L),
                     ...
                     mrow(pbox('All Teachers 0-9 Years', .16), 2L),
...
tbl = capture.output(print(xtable(
  ...
  align = 'p{.4\\textwidth}p{.2\\textwidth}p{.2\\textwidth}p{.2\\textwidth}'), 
...
idx = grep('years', tbl)
tbl = c(tbl[1L:(idx - 1L)],
        ' & \\multicolumn{4}{c}{Teacher Experience} \\\\',
        tbl[idx:length(tbl)])
