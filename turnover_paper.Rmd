...
#separate background data compilation to three steps
#  to cut down on re-run time if nothing affects the 
#  most time-consuming step (interpolation)
```
```{r interpolator, cache = TRUE, include = FALSE}
#for intermediate debugging to skip interpolation step
# payscales = fread(payscales, wds['data'] %+%
#                     "wisconsin_salary_scales_imputed.csv")
```
```{r complete_read, cache = TRUE, include = FALSE}
```
